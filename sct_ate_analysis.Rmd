...
justice_scores %>%
  ...
  group_by(score_type) %>%
  mutate(p_value = t.test(unlist(D), unlist(R))$p.value,
         t_value = t.test(unlist(D), unlist(R))$statistic,
         D_mean = round(mean(unlist(D), na.rm = T), 3),
         R_mean = round(mean(unlist(R)), 3)) %>%
  select(score_type, D_mean, R_mean, t_value, p_value) 
...
justice_scores %>%
 ...
 # geom_line(aes(x = value, y =  justiceTitle)) +
  ...
  labs(x = "Score", y = "Justice", 
      ...
      caption = "Voting data from the Supreme Court Database (20201 Version 1)") + 
  theme_minimal() +
...
justice_votes %>%
  ...
  labs(x = "Court Term", y = "Mean",
      ...
      caption = "Authorship and voting data from the Supreme Court Database (20201 Version 1)") +
  theme_classic()
