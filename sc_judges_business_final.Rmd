...
#   select(five_avg, term, bf_nyt_score) %>%
#   pivot_longer(!term, names_to = "scoreType", values_to = "score") %>%
#   ggplot(aes(x = term, y = score, color = scoreType)) + geom_line() 
```
...
court_justice_scores = court_monopoly_justice_score %>%
  ...
  group_by(type_case_score) %>%
  mutate(five_avg = rollmean(score, 5, align = "center", fill = 0)) %>%
...
court_case_scores = court_monopoly_case_score %>%
...
```{r psb and bcf over time figures}
court_justice_scores %>%
  filter(type_case_score == "pe_score" |  
 ...
 #     scale_color_grey(start = 0.25, end = 0.75) + 
  ...
  labs(x = "Court Term", y = "Moving average score",
       title = " Moving average measure of pro-enforcement and pro-large \ncompany bias over time (by justice)",
...
court_case_scores %>%
  ...
  filter(five_avg > 0) %>%
  ggplot() + 
  geom_line(aes(x = court_term, y = five_avg, color = type_case_score)) +
 #     scale_color_grey(start = 0.25, end = 0.75) + 
  theme_classic() +
  labs(x = "Court Term", y = "Moving average score",
       title = " Moving average measure of pro-enforcement and pro-large \ncompany bias over time (by case)",
       color = "Case sample") +   theme(legend.position="bottom", legend.box="vertical", legend.margin=margin())
...
antitrust_westlaw_sc_cases %>%
  ...
  theme_classic() + theme(legend.position = "bottom", legend.title = element_blank()) +
  labs(title = "Win-loss ratio of antitrust cases", 
       x = "Court Term",
       y = "Percent of cases that expand enforcement") +
    scale_y_continuous(breaks= breaks_pretty())
...
antitrust_westlaw_sc_cases %>%
  ...
  labs(title = "Win-loss ratio of antitrust cases", 
       x = "Court Term",
       y = "Percent of business vs. business cases that expand enforcement") +
    scale_y_continuous(breaks= breaks_pretty())
...
antitrust_westlaw_sc_cases %>%
  ...
  labs(title = "Win-loss ratio of antitrust cases", 
       x = "Court Term",
       y = "Percent of cases involving the government that expand enforcement") +
    scale_y_continuous(breaks= breaks_pretty())
