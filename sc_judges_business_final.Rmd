...
```{r bf roll mean - justice}
bf_scores_roll_mean = bf_scores_read %>%
  filter(BusinessLitigant == "1. business is Petitioner OR respondent" &
  ...
  rename(`pro-enforcement` = `j vote against business`,
         `anti-enforcement` = `j vote for business`) %>%
  ungroup() %>%
  mutate(five_avg = rollmean(score, 7, align = "center", fill = 0)) %>%
  relocate(five_avg) %>%
  filter(five_avg > 0) 
...
```
```{r roll mean bf_nyt - justice}
justice_bf_nyt_scores_roll_mean = bf_scores_read %>%
  filter(BusinessLitigant == "1. business is Petitioner OR respondent" &
           nytSalience == 1) %>%
  group_by(term, JVoteForBusiness) %>%
  summarise(votes = n()) %>%
 # filter(!is.na(JVoteForBusiness)) %>%
  ...
  rename(`pro-enforcement` = `j vote against business`,
         `anti-enforcement` = `j vote for business`) %>%
  ungroup() %>%
  mutate(five_avg = rollmean(score, 20, align = "center", fill = 0)) %>%
  relocate(five_avg) %>%
  filter(five_avg > 0) 


justice_bf_nyt_scores_roll_mean %>%
...
justice_scores %>%
  select(justiceLastName, justiceName) %>%
  group_by(justiceLastName) %>%
  summarise(count= n()) %>%
  filter(count != 5)

justice_scores %>%
  filter(justiceLastName == "Vinson")
